...
import { CommonActions, createNavigationContainerRef, DrawerActions, NavigationContainerRef, NavigationState, PartialState, RouteProp, StackActions, useNavigationContainerRef, useRoute, } from "@react-navigation/native";
...
export let navigationRef = useNavigationContainerRef<NavigationContainerRef<RouteParamList>>()//createNavigationContainerRef<NavigationContainerRef<RouteParamList>>();
/**
...
const jumpTo = async <T extends keyof RouteParamList>(navigation: any, name?: T, params?: RouteParamList[T]) => {
  if (!isFalsy(name)) {
...
function navigateAndReset11<T extends keyof RouteParamList>(
  name: T,
...
// function navigateAndResetWithDrawerClose<T extends keyof RouteParamList>(
//   name: T,
//   params?: RouteParamList[T]
// ) {
//   const drawerNavigator = navigationRef.current?.getRootState()?.routes.find(
//     (route) => route.name === 'drawerStack'
//   );
...
function navigateAndResetWithDrawerClose<T extends keyof RouteParamList>(
  ...
  if (navigationRef.isReady()) {
    navigationRef.dispatch(
      CommonActions.reset({
        index: 0, // Start at the first screen of the new stack
        routes: [
          {
            name, // Reset to the specified stack (e.g., 'authStack' or 'mainStack')
            params,
          },
        ],
      })
...
export {
  ...
  getPreviousRoute,
  pushTo,
  navigateAndResetWithDrawerClose,
  jumpTo
};
